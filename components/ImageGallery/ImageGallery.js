...
class ImageGallery extends React.Component {
    ...
    items.forEach((item, index) => {
      ...
      const slide = (
        <div
          ...
          key={`slide-${index}`}
          tabIndex="-1"
          className={styles[`image-gallery-slide ${alignment} ${originalClass}`]}
          style={slideStyle}
          onClick={onClick}
  ...
  renderThumbInner(item) {
    ...
    return (
      <span className={styles["image-gallery-thumbnail-inner"]}>
        <img
          className={styles["image-gallery-thumbnail-image"]}
          src={item.thumbnail}
        ...
        {
          item.thumbnailLabel && (
            <div className={styles["image-gallery-thumbnail-label"]}>
              {item.thumbnailLabel}
            </div>
          )
        }
  ...
  render() {
    ...
    const slideWrapper = (
      <div ref={this.imageGallerySlideWrapper} className={slideWrapperClass}>
        {renderCustomControls && renderCustomControls()}
        {
          this.canSlide() ? (
            <React.Fragment>
              ...
              <SwipeWrapper
                className={styles["image-gallery-swipe"]}
                delta={0}
                onSwiping={this.handleSwiping}
                onSwiped={this.handleOnSwiped}
              >
                <div className={styles["image-gallery-slides"]}>
                  {slides}
                </div>
              </SwipeWrapper>
            </React.Fragment>
        ...
        {
          showBullets && (
            <div className={styles["image-gallery-bullets"]}>
              <div
                className={styles["image-gallery-bullets-container"]}
                role="navigation"
        ...
        <div className={igContentClass}>
          {(thumbnailPosition === 'bottom' || thumbnailPosition === 'right') && slideWrapper}
          {
            showThumbnails && thumbnails.length > 0 ? (
              ...
              >
                <div className={styles["image-gallery-thumbnails"]} ref={this.thumbnailsWrapper} style={this.getThumbnailBarHeight()}>
                  <div
                    ref={this.thumbnails}
